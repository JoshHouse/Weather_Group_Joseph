...
def get_weather():
    ...
    city_encoded = urllib.parse.quote(city_decoded)
    ...
    if city_encoded in weather_cache:
        # Record the cashed information for that city
        cached_data = weather_cache[city_encoded]
        ...
        if current_time - cached_data["timestamp"] < CACHE_EXPIRATION:
            ...
            del weather_cache[city_encoded]
    ...
    url = f"{API_URLS['WEATHER']}?q={city_encoded}&appid={API_KEYS['JOSHUA']}&units={units}"
    ...
    if response.status_code == 200:
        ...
        weather_cache[city_encoded] = {"timestamp": current_time, "data": weather_data, "units": units}
        ...
        app.logger.info("Fetching weather with URL:" + "?city=" + city_encoded + "&units=" + units)
        return jsonify({"error": "Failed to fetch weather data from API"}), response.text
